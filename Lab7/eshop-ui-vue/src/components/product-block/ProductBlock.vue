<template>
  <div v-if="data" class="product-block">
    ...
    <div class="productItems"
         ...
         v-bind:class="{mainItems: dataType === 'Offering' || !dataType, filteredItems: dataType === 'ProductCategory'}">
      <ProductItem v-for="product in products" v-bind:key="product.id" v-bind:product="product"/>
    </div>
    <div class="emptyProductItems" v-else>
      <h1>Sorry, nothing found in stock by your request</h1>
      <img src="@/assets/img/empty-stock.png" alt="Empty Stock"/>
    </div>
  </div>
</template>
...
import {sortArray, groupBy} from "@/utils/list-utils";
    ...
    products() {
      ...
      if (this.dataType === 'Offering' && this.filteringCategories && this.filteringCategories.length > 0) {
        ...
        productsResult = productsResult.filter(product => {
          if (!product.characteristics || product.characteristics.length === 0) return false;
          const filteringCharacteristicGroups = groupBy(this.filteringCharacteristics, 'groupId');
          const productCharacteristicGroups = groupBy(product.characteristics, 'groupId');
          for (const [key, value] of Object.entries(filteringCharacteristicGroups)) {
            if (!productCharacteristicGroups[key]) return false;
            const filteringCharacteristicIds = value.map(c => c.id);
            const productCharacteristicIds = productCharacteristicGroups[key].map(c => c.id);
            if (!filteringCharacteristicIds.some(id => productCharacteristicIds.includes(id))) return false;
          }
          return true;
        });
      }
